...
function ambilDataFarmasi($status, $koneksi, $IDRUANGAN, $isOrder = false) {
    if ($isOrder) {
        ...
        SELECT 
            ...
            (SELECT r2.DESKRIPSI FROM pendaftaran.kunjungan k2 
             ...
             AND (r2.DESKRIPSI LIKE '%Poli%' OR r2.DESKRIPSI LIKE '%UGD%' OR r2.DESKRIPSI LIKE '%IGD%')
            ...
            (SELECT CONCAT(COALESCE(res.POS_ANTRIAN, 'A'), IF(res.CARABAYAR = 2, '2', '1'), '-', LPAD(res.NO, 3, '0')) 
        ...
        SELECT 
            ...
            (SELECT MAX(CASE WHEN odr.RACIKAN = 1 THEN 1 ELSE 0 END) FROM layanan.order_resep or2 
             JOIN layanan.order_detil_resep odr ON odr.ORDER_ID = or2.NOMOR WHERE or2.KUNJUNGAN = k.NOMOR) AS RACIKAN,
            -- CARI NOMOR ANTREAN BERDASARKAN NORM
            (SELECT CONCAT(COALESCE(res.POS_ANTRIAN, 'A'), IF(res.CARABAYAR = 2, '2', '1'), '-', LPAD(res.NO, 3, '0')) 
             FROM regonline.reservasi res 
             WHERE res.NORM = p.NORM 
    ...
    if($result) {
        while ($row = $result->fetch_assoc()) {
            ...
            if (!$row['NO_ANTRIAN']) $row['NO_ANTRIAN'] = "-";
            if (!$row['ASAL_RUANGAN']) $row['ASAL_RUANGAN'] = "Unit Terkait";
