...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        #header-top {
            background: var(--bg-dark); color: white; padding: 15px 30px;
            font-size: 22px; font-weight: 800; border-bottom: 5px solid var(--accent-yellow);
            display: flex; justify-content: space-between; align-items: center;
            box-shadow: 0 4px 15px rgba(0,0,0,0.3);
        }

        .main-layout { display: flex; height: calc(100vh - 65px); padding: 20px; gap: 20px; }
        .left-section { flex: 1; display: flex; flex-direction: column; gap: 20px; }

        .video-container {
            flex: 1; background: black; border-radius: 20px; overflow: hidden;
        ...
        #nama-dipanggil {
            ...
            border-top: 6px solid var(--accent-yellow); box-shadow: 0 10px 25px rgba(0,0,0,0.2);
            display: flex; flex-direction: column; justify-content: center;
        }
        #antrean-panggilan { font-size: 90px; font-weight: 900; color: var(--accent-yellow); line-height: 1; margin: 10px 0; }
        #nama-panggilan { font-size: 38px; font-weight: 800; text-transform: uppercase; }

        .antrian-container { flex: 2; display: flex; gap: 20px; }
        .column {
            flex: 1; background: rgba(255, 255, 255, 0.95); border-radius: 25px;
            display: flex; flex-direction: column; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.2);
        }
        .col-header { text-align: center; padding: 15px; background: var(--primary-green); color: white; }
        .col-header .counter { font-size: 60px; font-weight: 800; display: block; color: var(--accent-yellow); line-height: 1; }
        
        .column-content { flex: 1; position: relative; overflow: hidden; padding: 15px; }
        ...
        @keyframes moveUp {
            ...
            100% { transform: translateY(-50%); } /* Naik tepat setengah dari total tinggi wrapper */
        }

        .item {
            background: white; margin-bottom: 15px; padding: 15px; border-radius: 15px;
            border-left: 10px solid #cbd5e1; box-shadow: 0 4px 6px rgba(0,0,0,0.05);
        ...
        .item .patient-name { font-size: 20px; font-weight: 800; text-transform: uppercase; padding-right: 120px; }
        .item .room-origin { font-size: 14px; color: #64748b; font-weight: 600; }
        .item .time-info { font-size: 12px; color: #94a3b8; }

        .no-antrian-box {
            position: absolute; top: 10px; right: 10px; background: var(--accent-yellow);
            color: #000; font-size: 34px; font-weight: 900; padding: 5px 15px;
            border-radius: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.2); border: 3px solid #ca8a04;
        }

        .badge { font-size: 11px; font-weight: 800; padding: 4px 10px; border-radius: 6px; display: inline-block; margin-bottom: 5px; }
...
<body>
    ...
    <div id="header-top">
        <span>ANTRIAN FARMASI RSUD GIRI EMAS</span>
    ...
    <div class="main-layout">
        <div class="left-section">
            <div class="video-container">
                <iframe src="https://www.youtube.com/embed/TK6qVT2w0W0?autoplay=1&mute=1&loop=1&playlist=TK6qVT2w0W0" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>
            </div>
        ...
        <div class="antrian-container">
            <div class="column"><div class="col-header"><span class="counter" id="jumlah-belum">000</span><span class="label">Belum Dilayani</span></div><div class="column-content" id="cont-belum"></div></div>
            <div class="column"><div class="col-header"><span class="counter" id="jumlah-dilayani">000</span><span class="label">Sedang Dilayani</span></div><div class="column-content" id="cont-dilayani"></div></div>
            <div class="column"><div class="col-header"><span class="counter" id="jumlah-selesai">000</span><span class="label">Selesai / Siap</span></div><div class="column-content" id="cont-selesai"></div></div>
        </div>
    </div>
...
function loadAntrian() {
    ...
    .then(data => {
        ...
        const categories = { "belum_diterima": "#cont-belum", "dilayani": "#cont-dilayani", "selesai": "#cont-selesai" };
        ...
        Object.keys(categories).forEach(key => {
            ...
            if (data[key]) {
                filteredList = data[key].filter(item => {
                    ...
                    return !asal.includes("UNIT TERKAIT") && !asal.includes("PERAWATAN") && !asal.includes("INAP");
                });
            }
            ...
            if(filteredList.length > 0) {
                filteredList.forEach(item => {
                    ...
                    let badge = item.RACIKAN == "1" ? '<span class="badge badge-r">RACIKAN</span>' : '<span class="badge badge-nr">NON RACIK</span>';
        ...
        document.getElementById("jumlah-belum").textContent = String(data.total.belum_diterima).padStart(3,'0');
        document.getElementById("jumlah-dilayani").textContent = String(data.total.dilayani).padStart(3,'0');
...
function cekPanggilan() {
    fetch("last_panggilan.txt?rnd=" + Math.random()).then(res => res.text()).then(txt => {
        ...
        if (hash !== lastHash) {
            ...
            el.classList.remove("animate__fadeIn"); void el.offsetWidth; el.classList.add("animate__fadeIn");
...
function formatSuara(teks) {
    ...
    hasil = hasil.replace(/\bUGD\b/g, "Unit Gawat Darurat").replace(/\bIGD\b/g, "Instalasi Gawat Darurat").replace(/\bPOLI\b/g, "Poliklinik");
...
function panggilSuara(nama, asal, nomor) {
    let kalimat = `Nomor Antrean , ${ejaNomorAntrean(nomor)} , , Keluarga pasien , ${formatSuara(nama)} , dari , ${formatSuara(asal)} , silakan mengambil obat di loket apotek.`;
